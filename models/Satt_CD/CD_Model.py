...
from losses.Edge_BCEDiceLoss import edgeBCE_Dice_loss
# from losses.dist_angle import DAngleLoss
# import losses.GFCDLoss as GFLoss
...
class CD_Model(BaseModel):
    def __init__(self, opt):
        ...
        self.is_adv_train=train_opt['is_adv_train']
        if train_opt['is_adv_train']:
            ...
            self.netD_seg.train()
            #self.netD_MS.train()
        self.use_dangle=opt["network_G_CD"]["use_dangle"]
        ...
        #==================use ohem bce_dice==================================
        # self.cri_seg_loss = edgeBCE_Dice_loss(thresh=0.5).to(self.device)
        # self.cri_seg0_loss = edgeBCE_Dice_loss(thresh=0.5).to(self.device)
        ...
        if self.netG:
            ...
            self.optimizer_G =torch.optim.AdamW(optim_params, lr=train_opt['lr_G'], weight_decay=wd_G)
        ...
        if train_opt['lr_scheme'] == 'MultiStepLR':
            ...
            def lambda_rule(epoch):
                lr_l = 1.0 - epoch / float(train_opt["nepoch"] + 1)
                # lr_l = 1.1 ** (epoch + 10)
                # lr_l = 0.98 ** epoch
                return lr_l

            for optimizer in self.optimizers:
                self.schedulers.append(torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule))
            ...
            if train_opt["warmup_epoch"]>0:
                ...
                for optimizer in self.optimizers:
                    ...
                    self.schedulers.append(lr_scheduler.CosineAnnealingLR(optimizer, \
                                                                       T_max=train_opt["nepoch"]//train_opt["cosT"]))#T_max可以看做coisne函数的半个周期长度
        ...
        elif train_opt['lr_scheme'] == 'CosineRe':
            if train_opt["warmup_epoch"] > 0:

                from torch_warmup_lr import WarmupLR

                for optimizer in self.optimizers:
                    my_scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=train_opt["nepoch"] // train_opt[
                                                                              "cosT"],T_mult=2)

                    my_scheduler = WarmupLR(my_scheduler, init_lr=train_opt['lr_G'] * 0.01,
                                            num_warmup=train_opt["warmup_epoch"], warmup_strategy='cos')
                    self.schedulers.append(my_scheduler)
                ...
                for optimizer in self.optimizers:
                    self.schedulers.append(lr_scheduler.CosineAnnealingWarmRestarts(optimizer, \
                                                                          T_0=train_opt["nepoch"] // train_opt[
                                                                              "cosT"],T_mult=2))  # T_max可以看做coisne函数的半个周期长度
                    # my_scheduler =torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
                    #     optimizer,
                    #     T_0=2,  # T_0就是初始restart的epoch数目
                    #     T_mult=2,  # T_mult就是重启之后因子,即每个restart后，T_0 = T_0 * T_mult
        ...
        elif train_opt["lr_scheme"]=='PolyCycLR':
            from utils.torch_poly_cyc_lr import PolynomialLRDecay
            #PolynomialLRDecay(optim, global_steps=200,decay_steps=100, end_lr=0.00001, power=2,Cycle=True)
            for optimizer in self.optimizers:
                self.schedulers.append(PolynomialLRDecay(optimizer, global_steps=train_opt["nepoch"],decay_steps=train_opt["nepoch"]//3, end_lr=0.0000001, power=2,Cycle=True))

        elif train_opt["lr_scheme"]=='PolyLR':
            from utils.torch_poly_cyc_lr import PolynomialLRDecay
            #PolynomialLRDecay(optim, global_steps=200,decay_steps=100, end_lr=0.00001, power=2,Cycle=True)
            for optimizer in self.optimizers:
                self.schedulers.append(PolynomialLRDecay(optimizer, global_steps=train_opt["nepoch"],decay_steps=train_opt["nepoch"]//3, end_lr=0.0000001, power=2,Cycle=False))

        elif train_opt["lr_scheme"]=='PolyStepLR':
            from utils.torch_poly_cyc_lr import PolynomialLRDecay
            #PolynomialLRDecay(optim, global_steps=200,decay_steps=100, end_lr=0.00001, power=2,Cycle=True)
            for optimizer in self.optimizers:
                self.schedulers.append(PolynomialLRDecay(optimizer, global_steps=train_opt["niter"],decay_steps=train_opt["niter"], end_lr=0.0000001, power=0.9,Cycle=False))
        ...
        #=======================for teacher network if needed=====================
        if self.config["train"]["use_tea"]:
            self.netG_tea = networks.define_G_CD(opt).to(self.device)
            ...
            self.optimizer_G_Tea = EMAWeightOptimizer(self.netG_tea, self.netG, opt["train"]["tea_alpha"])
        elif self.config["train"]["use_tea2"]:
            self.netG_tea = networks.define_G_CD(opt).to(self.device)
            self.netG_tea2 = networks.define_G_CD(opt).to(self.device)
            for p in self.netG_tea.parameters():
                p.requires_grad = False
            for p in self.netG_tea2.parameters():
                p.requires_grad = False
        else:
            self.netG_tea=None
            self.optimizer_G_Tea=None
        #load_network(self, load_path, network, strict=False):
        logger.info(Logger.dict2str(opt))#print model config
        self.print_network()#print model module
        if train_opt["use_checkpoint"]:
            model_path = self.config.model_dir + '/' + self.config["train"]["pretrained_path"] + '_best_acc.pth'
            self.load_network(model_path,self.netG)
    ...
    def reset_models(self):
        ...
        self.optimizers=[]
        self.schedulers=[]
        self.netG = networks.define_G_CD(self.config).to(self.device)
        self.optimizer_G = torch.optim.AdamW(self.netG.parameters(), lr=self.train_opt['lr_G'], weight_decay=self.train_opt['weight_decay_G'])
    ...
    def feed_data(self, data):
        ...
        if self.config["train"]["use_edge_loss"]:
            self.edge=data["edge"].to(self.device)
        ...
        if self.config["network_G_CD"]["use_dangle"]:
            self.dist,self.angle=data['dist'],data['angle']
            self.dist_map,self.angle_map=self.dist.to(self.device),self.angle.to(self.device)
...
0.000124
    ...
    def optimize_parameters_SC2(self, step):
        # =============G==================
        #print("using binary map for aux...")
        ...
        if self.config["network_G_CD"]["use_DS"]:
            ...
            if self.config["network_G_CD"]["which_model_G"]=="CTFINet":
                preds= self.netG(images_T1, images_T2)
                for i in range(len(preds)):
                    preds_prob = F.softmax(preds[i], dim=1)
                    outputs = preds_prob[:, 1].unsqueeze(1).float()
                    l_g_total += self.cri_seg_loss(outputs, labels)
                ...
                labels1 = F.interpolate(labels.float(), (img_down_size, img_down_size), mode='bilinear',
                                        align_corners=True)
                labels2 = F.interpolate(labels.float(), (img_down_size * 2, img_down_size * 2), mode='bilinear',
                                        align_corners=True)
                labels3 = F.interpolate(labels.float(), (img_down_size * 4, img_down_size * 4), mode='bilinear',
                ...
                l_g_total += self.cri_seg_loss(pred1, labels1) + self.cri_seg_loss(pred2, labels2) + self.cri_seg_loss(
                    pred3, labels3) + \
                             self.cri_seg_loss(pred4, labels)


        else:
            pred= self.netG(images_T1, images_T2)
            if self.config["train"]["use_edge_loss"]:
                l_g_total = edgeBCE_Dice_loss(pred, labels, self.edge)
            else:
                l_g_total = self.cri_seg_loss(pred, labels)
            #l_g_total= self.cri_seg_loss(pred, labels)
    ...
    def optimize_parameters_SC1(self, step):
        # =============G==================
        #print("using binary map for aux...")
        ...
        if self.config["train"]["clip_grad"]:
            torch.nn.utils.clip_grad_norm_(self.netG.parameters(),2)
    ...
    def optimize_parameters_CDFormer(self, step):
        # =============G==================
        #print("using binary map for aux...")
        ...
        if self.config["network_G_CD"]["use_DS"]:
            ...
            if self.config["network_G_CD"]["in_c"]==3:
                if self.use_dangle:
                    preds,pred_mask,pred_dir=self.netG(images_T1, images_T2)
                else:
                    preds = self.netG(images_T1, images_T2)

            else:
                preds = self.netG(images_T12)
            if class_num == 2:
                if isinstance(preds,list):
                    ...
                    for i in range(len(preds)):
                        preds_prob = F.softmax(preds[i], dim=1)
                        outputs = preds_prob[:, 1].unsqueeze(1).float()
                        l_g_total+=self.cri_seg_loss(outputs, labels)*loss_weight[i]
                    l_g_total/=len(preds)
                else:
                   preds_prob = F.softmax(preds, dim=1)
                   outputs = preds_prob[:, 1].unsqueeze(1).float()
                   if self.use_dangle:
                       # seg_logit, pred_mask, pred_direction, seg_label, distance_map, angle_map
                       l_g_total, _ = self.cri_seg_loss(outputs, pred_mask, pred_dir, labels, self.dist_map,
                                                        self.angle_map)
                   else:
                       l_g_total = self.cri_seg_loss(outputs, labels)
            else:
                #l_g_total = self.cri_seg_loss(preds, labels)
                if self.config["train"]["use_edge_loss"]:
                    l_g_total = edgeBCE_Dice_loss(preds, labels, self.edge)
                else:
                    l_g_total = self.cri_seg_loss(preds, labels)
    ...
    def save_best_acc(self,fold_index=0):
        #model_path=self.config.model_name
        if self.config.use_KFold:
            model_path = self.config.model_dir + '/' + self.config.pred_name + '_fold'+str(self.config.fold_index)+'_best_acc.pth'
        else:
    ...
    def save_latest(self,epoch=0):
        model_path = self.config.model_dir + '/' + self.config.pred_name + 'latest.pth'
        if self.config["train"]["use_tea"]:
            checkpoint = {
                'model': self.netG.state_dict(),
                'model_tea':self.netG_tea.state_dict(),
                'optimizer': self.optimizer_G.state_dict(),
                #'optimizer_tea':self.optimizer_G_Tea.state_dict(),#no need
                'epoch': epoch
            }
        else:
            checkpoint = {
...
if __name__ == '__main__':
    ...
    opt_path = r'E:/TEST/MyKeras/PyTorch/Scene_Seg/configs/config.json'
