...
def one_hot2dist(seg):
    res = np.zeros_like(seg)  # [1,100,100]
    for i in range(len(seg)):
        posmask = seg[i].astype(np.bool)  # [100,100]
        if posmask.any():
            negmask = ~posmask
            res[i] = distance(negmask) * negmask - (distance(posmask) - 1) * posmask
    return res  # [1,100,100]


def class2one_hot(seg, C):
    seg = seg.unsqueeze(dim=0) if len(seg.shape) == 2 else seg  # [1,100,100]
    res = torch.stack([seg == c for c in range(C)], dim=1).type(torch.int32)  # [1,1,100,100]
    return res


# Active Boundary Loss https://github.com/wangchi95/active-boundary-loss
class ABL(nn.Module):
    def __init__(self, isdetach=True, max_N_ratio=1 / 100, ignore_label=-1, label_smoothing=0.2, weight=None,
                 max_clip_dist=20.):
    ...
    def logits2boundary(self, logit):
        ...
        kl_lr = kl_div(logit[:, :, 1:, :], logit[:, :, :-1, :]).sum(1, keepdim=True)  # [1,1,99,100]
        kl_ud = kl_div(logit[:, :, :, 1:], logit[:, :, :, :-1]).sum(1, keepdim=True)  # [1,1,100,99]
        kl_lr = torch.nn.functional.pad(
            kl_lr, [0, 0, 0, 1, 0, 0, 0, 0], mode='constant', value=0)  # [1,1,100,100]
        kl_ud = torch.nn.functional.pad(
            kl_ud, [0, 1, 0, 0, 0, 0, 0, 0], mode='constant', value=0)
        kl_combine = kl_lr + kl_ud  # [1,1,100,100]
        while True:  # avoid the case that full image is the same color
            kl_combine_bin = (kl_combine > eps).to(torch.float)  # [1,1,100,100]
        ...
        edge2 = torch.nn.functional.conv2d(kl_combine_bin, dilate_weight, stride=1, padding=1)  # [1,1,100,100]
        edge2 = edge2.squeeze(1)  # NCHW->NHW [1,100,100]
        kl_combine_bin = (edge2 > 0)
        return kl_combine_bin

    def gt2boundary(self, gt, ignore_label=-1):  # gt NHW
        gt_lr = gt[:, 1:, :] - gt[:, :-1, :]  # NHW  [1,99,100]
        gt_ud = gt[:, :, 1:] - gt[:, :, :-1]  # [1,100,99]
        '''
...
第二行的（3,4），意义是对倒数第二个维度（dim=-2），前面填充3个单位，后面填充4个单位
        ...
        '''
        gt_lr = torch.nn.functional.pad(gt_lr, [0, 0, 0, 1, 0, 0], mode='constant', value=0) != 0  # bool [1,100,100]
        gt_ud = torch.nn.functional.pad(gt_ud, [0, 1, 0, 0, 0, 0], mode='constant', value=0) != 0  # bool [1,100,100]
        gt_combine = gt_lr + gt_ud  # [1,100,100]
        del gt_lr
        del gt_ud

        # set 'ignore area' to all boundary
        gt_combine += (gt == ignore_label)  # bool
    ...
    def get_direction_gt_predkl(self, pred_dist_map, pred_bound, logits):
        ...
        bound = torch.nonzero(pred_bound * 1)  # [492,3]
        n, x, y = bound.T
        max_dis = 1e5
    ...
    def get_dist_maps(self, target):
        target_detach = target.clone().detach()  # bool [1,100,100]
        dist_maps = torch.cat(
            [self.dist_map_transform(target_detach[i]) for i in range(target_detach.shape[0])])  # [1,100,100]
        out = -dist_maps
    ...
    def forward(self, logits, target):
        ...
        eps = 1e-10
        ph, pw = logits.size(2), logits.size(3)  # 100,100
        h, w = target.size(1), target.size(2)  # 100,100

        if ph != h or pw != w:
            logits = F.interpolate(input=logits, size=(
                h, w), mode='bilinear', align_corners=True)

        gt_boundary = self.gt2boundary(target, ignore_label=self.ignore_label)  # bool [1,100,100]

        dist_maps = self.get_dist_maps(
            gt_boundary).cuda()  # [1,100,100] <-- it will slow down the training, you can put it to dataloader.

        pred_boundary = self.logits2boundary(logits)  # [1,100,100]
...
if __name__ == '__main__':
    ...
    n, c, h, w = 1, 2, 100, 100
    gt = torch.zeros((n, h, w)).cuda()  # [1,100,100]
    gt[0, 5] = 1  # equals to gt[0,5,:] 5th row of gt with 100 elements
    gt[0, 50] = 1
    logits = torch.randn((n, c, h, w)).cuda()  # [1,2,100,100]
